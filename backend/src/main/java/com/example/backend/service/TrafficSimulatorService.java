...
public class TrafficSimulatorService {
    ...
    public synchronized String startSimulation(int messagesPerSecond) {
        ...
        // Seed test sanctions data for compliance testing
        seedTestSanctions();
    ...
    private String generateRealisticPacs008Message() {
        ...
        // Chance of sanctions-triggering name (10% for testing - can be reduced for production-like simulation)
        // This ensures we get blocked transactions for testing the Investigations page
        ...
        // Update account owner names to match transaction party names
        // This ensures compliance screening uses the correct names from the transaction
        updateAccountNameIfNeeded(senderIban, senderName);
        if (!senderIban.equals(receiverIban)) {
            updateAccountNameIfNeeded(receiverIban, receiverName);
        }
    ...
    /**
     * Seed test sanctions data for compliance testing.
     * This ensures the sanctions database contains entries that match the names
     * generated by the traffic simulator, enabling proper compliance blocking.
     */
    @Transactional
    private void seedTestSanctions() {
        try {
            // Check if sanctions already exist to avoid duplicate seeding
            long existingCount = sanctionsRepository.count();
            if (existingCount > 0) {
                logger.debug("Sanctions database already contains {} entries, skipping seed", existingCount);
                // Still refresh BK-tree in case new sanctions were added
                fuzzyMatchService.refreshBKTree();
                return;
            }

            logger.info("Seeding test sanctions data for simulation");

            // Seed sanctions matching the POTENTIAL_SANCTIONS_NAMES list
            for (String name : POTENTIAL_SANCTIONS_NAMES) {
                // Normalize name for matching (same logic as FuzzyMatchService)
                String normalizedName = normalizeNameForSanctions(name);
                ...
                if (existing.isEmpty()) {
                    ...
                    sanction.setNormalizedName(normalizedName);
                    sanction.setSource("SIMULATION_TEST");
                    sanction.setRiskScore(100); // High risk for test data
                    sanctionsRepository.save(sanction);
                    logger.debug("Seeded test sanction: {}", name);
                }
            }

            // Refresh BK-tree with new sanctions data
            fuzzyMatchService.refreshBKTree();
            logger.info("Test sanctions seeding completed. Total sanctions: {}", sanctionsRepository.count());

        } catch (Exception e) {
            logger.error("Failed to seed test sanctions: {}", e.getMessage(), e);
            // Don't fail simulation if seeding fails, but log the error
        }
    }

    /**
     * Normalize name for sanctions matching (matches FuzzyMatchService logic).
     * - Convert to uppercase
     * - Remove punctuation and special characters
     * - Collapse whitespace
     */
    private String normalizeNameForSanctions(String name) {
        if (name == null) {
            return "";
        }
    ...
    /**
     * Update account owner name if it differs from the transaction party name.
     * This ensures compliance screening uses the actual transaction party names.
     * Only updates in simulation mode to keep account names synchronized with transaction data.
     * Note: JpaRepository.save() handles transactions internally.
     */
    private void updateAccountNameIfNeeded(String iban, String partyName) {
        if (iban == null || partyName == null || partyName.trim().isEmpty()) {
            return;
        }
        ...
        try {
            ...
            if (accountOpt.isPresent()) {
                ...
                // Only update if the name is different (avoid unnecessary writes)
                if (!partyName.equals(account.getOwnerName())) {
                    String oldName = account.getOwnerName();
                    account.setOwnerName(partyName);
                    account.setUpdatedAt(LocalDateTime.now());
                    accountRepository.save(account);
                    logger.debug("Updated account {} owner name from '{}' to '{}'", 
                                iban, oldName, partyName);
                }
            } else {
                logger.warn("Account not found for IBAN: {}. Cannot update owner name.", iban);
            }
        } catch (Exception e) {
            // Log but don't fail the simulation if account update fails
            logger.warn("Failed to update account name for IBAN {}: {}", iban, e.getMessage());
        }
    }
